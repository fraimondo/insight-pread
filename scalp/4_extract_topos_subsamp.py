...
for condition in ['EC', 'EO']:
    for subsamp in [16, 32, 64, 128]:
        out_dir = op.join('data', 's_{}'.format(subsamp))
        if not op.exists(out_dir):
            os.mkdir(out_dir)
        out_dir = op.join(out_dir, condition)
        if not op.exists(out_dir):
            os.mkdir(out_dir)

        results_dir = op.join(db_path, 'results', 's_{}'.format(subsamp))
        
        results_dir = op.join(results_dir, condition)
        reductions=['aramis/insight/egi{}/trim_mean80'.format(subsamp),
                    'aramis/insight/egi{}/std'.format(subsamp)]
        summary = summarize_run(
            in_path=results_dir, recompute=False,
            reductions=reductions)
        summary = summary.filter(subjects=df_map['EEG_ID'].values)
    
        summary.save(op.join('data', 'summary', condition, 
                             's_{}'.format(subsamp)))
        c_names = ['E{}'.format(x) for x in range(1, subsamp + 1)]
        ...
        for this_reduction, this_topo in summary.topos().items():
            ...
            for i_marker, marker in enumerate(summary._topo_names):
                ...
                df.to_csv(op.join(out_dir, fname), sep=';')