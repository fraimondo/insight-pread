...
from nice_ext.equipments import map_montage
...
sys.path.append('../')
from common import get_scalp_markers, get_scalp_reductions, map_montage
...
for condition in ['EC', 'EO']:
    try:
        ...
        logger.info(msg + '\nOpening subject epochs failed ("%s")' % subject)
        sys.exit(-5)
    ...
    for subsamp in [16, 32, 64, 128]:
        ...
        mne.utils.set_log_file(
            op.join(results_dir, subject, subject + log_suffix))


        logger.info('Running {}'.format(subject))
        report = None
        montage = 'egi/{}'.format(subsamp)
        if subsamp < 128:
            montage += 'a'
        try:

            new_epochs = map_montage(epochs, montage)
